...
Осознанно не входит в этот слайс: мастер настройки (`prompt-14`), UI вкладки «Параметры
подключения» (Фаза 8.4), финализация списка ценовых полей (SPEC §9.5 — маппинг спроектирован
так, чтобы список рос без изменений кода адаптеров).
