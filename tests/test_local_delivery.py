...
class LocalDeliveryGitTests(unittest.TestCase):
    ...
    def test_windows_entrypoint_is_ascii_only_and_uses_isolated_checkout(self) -> None:
        ...
        self.assertIn(b"%~dp0region-price-monitor-g01", data)
        self.assertNotIn(b"%~dp0region-price-monitor\"", data)
        self.assertIn(b"--single-branch", data)
        self.assertNotIn(b"reset --hard", data)
        self.assertNotIn(b"git clean", data)
